...
@app.route('/')
def index():
    cursor.execute('SELECT title, price FROM products ORDER BY price ASC')
...
def search():
        if request.method == 'POST':
            ...
            title_sql_query = 'SELECT title, price FROM products WHERE title LIKE ? COLLATE NOCASE'
            title_params = (f'%{title_query}%',)
            cursor.execute(title_sql_query, title_params)
            title_results = cursor.fetchall()

            color_query = parts[-1]
            color_sql_query = 'SELECT title, price FROM products WHERE color LIKE ? COLLATE NOCASE'
            color_params = (f'%{color_query}%',)
            cursor.execute(color_sql_query, color_params)
            color_results = cursor.fetchall()

            combined_sql_query = 'SELECT title, price FROM products WHERE title LIKE ? COLLATE NOCASE AND color LIKE ? COLLATE NOCASE'
            combined_params = (f'%{title_query}%', f'%{color_query}%')
            cursor.execute(combined_sql_query, combined_params)
            combined_results = cursor.fetchall()


            if len(parts) >= 2:
                multi_title_query = ' '.join(parts[:-1])
                multi_title_sql_query = 'SELECT title, price FROM products WHERE title LIKE ? COLLATE NOCASE'
            ...
            all_results = title_results + color_results + combined_results + multi_title_results
            unique_results = []
            unique_titles_set = set()
            for title, price in all_results:
                title_lower = title.lower()
                if title_lower not in unique_titles_set:
                    unique_titles_set.add(title_lower)
                    unique_results.append((title, price))
